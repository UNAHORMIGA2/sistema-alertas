...
export default function AmbulanceDashboardScreen({ navigation }) {
  ...
  const pendingAlerts = useMemo(() => alerts.filter((item) => (item?.tipo || "").toLowerCase() === "medica" && item?.estado !== "cerrada"), [alerts]);
  ...
  useEffect(() => {
    ...
    if (available) {
      startLocationUpdates(async (coords) => {
        ...
        .then((subscription) => { watcher = subscription; })
        .catch(() => { watcher = null; });
    }
  ...
  const handleAccept = async (alerta) => {
    ...
    if (!alertId) return Alert.alert("Error", "La alerta no tiene identificador valido");
    try {
      await api.post(`/mobile/asignaciones/${alertId}/aceptar`);
      navigation.navigate("AmbulanceAlert", { alert: { ...alerta, estado: "asignada" } });
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => loadAlerts(true)} />}>
      ...
      <View style={styles.headerRow}>
        <View>
          <Text style={styles.headerTitle}>Panel de Ambulancia</Text>
          <Text style={styles.agentText}>Unidad activa</Text>
        </View>
        <Pressable onPress={() => navigation.navigate("AmbulanceProfile")}><Ionicons name="heart-outline" size={22} color="#DC2626" /></Pressable>
      </View>
      ...
      <Text style={styles.sectionTitle}>Emergencias Medicas Pendientes de Confirmacion ({pendingAlerts.length})</Text>

      {topAlert ? (
        <View style={styles.alertCard}>
          <View style={styles.alertCardHeader}>
            <Text style={styles.alertCardTitle}>Emergencia Medica</Text>
            <View style={styles.badge}><Text style={styles.badgeText}>{topAlert?.estado || "pendiente"}</Text></View>
          </View>
          ...
          <Pressable
            ...
            onPress={() => {
              if (isAssignedToMe) {
                navigation.navigate("AmbulanceAlert", { alert: topAlert });
                return;
              }
        ...
        <View style={styles.emptyCard}><Text style={styles.emptyTitle}>Emergencias asignadas</Text><Text style={styles.emptyText}>Sin alertas por ahora. Recuerda: nuevas alertas tardan aprox. 30s en activarse.</Text></View>
      )}

      <View style={styles.statsGrid}>
        <View style={styles.statCard}><Text style={styles.statTitle}>Alertas Activas</Text><View style={styles.statRow}><Text style={styles.statValue}>{activeCount}</Text><MaterialIcons name="graphic-eq" size={24} color="#FB923C" /></View></View>
        <View style={styles.statCard}><Text style={styles.statTitle}>En Espera</Text><View style={styles.statRow}><Text style={styles.statValue}>{waitingCount}</Text><Ionicons name="alert-circle" size={24} color="#EF4444" /></View></View>
        <View style={styles.statCard}><Text style={styles.statTitle}>Completadas</Text><View style={styles.statRow}><Text style={styles.statValue}>{closedCount}</Text><Ionicons name="checkmark-circle" size={24} color="#22C55E" /></View></View>
      </View>

      <View style={styles.instructionsCard}>
        <Text style={styles.instructionsTitle}>Instrucciones de Operacion</Text>
        <Text style={styles.instructionsItem}>- Mantente en estado "Disponible" durante tu guardia activa.</Text>
        <Text style={styles.instructionsItem}>- Responde alertas dentro del SLA medico institucional.</Text>
        <Text style={styles.instructionsItem}>- Actualiza estado de la alerta al llegar al incidente.</Text>
        <Text style={styles.instructionsItem}>- Completa reporte clinico al finalizar atencion.</Text>
      </View>

      <HamburgerMenu visible={menuVisible} onClose={() => setMenuVisible(false)} title="Menu ambulancia" items={menuItems} />
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  statusCard: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#86EFAC", borderRadius: 12, padding: 10, marginBottom: 12 },
  ...
  alertCard: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#F97316", borderRadius: 14, padding: 10 },
  ...
  badge: { borderRadius: 999, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: "#EF4444" },
  ...
  primaryButton: { marginTop: 2, height: 40, borderRadius: 10, backgroundColor: "#22C55E", alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 6 },
  ...
  emptyText: { color: "#6B7280", borderWidth: 1, borderColor: "#FCA5A5", borderRadius: 10, paddingVertical: 8, paddingHorizontal: 8, textAlign: "center" },
  ...
  instructionsCard: { marginTop: 12, backgroundColor: "#EFF6FF", borderColor: "#3B82F6", borderWidth: 1, borderRadius: 12, padding: 11 },
