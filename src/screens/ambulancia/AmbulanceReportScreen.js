...
export default function AmbulanceReportScreen({ navigation, route }) {
  ...
  const pickImage = async () => {
    ...
    if (permission.status !== "granted") return Alert.alert("Permiso requerido", "Debes permitir acceso a galeria para adjuntar evidencia.");
    const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 0.8 });
    if (!result.canceled && result.assets?.[0]) setPhotos((prev) => [...prev, result.assets[0]]);
  };
